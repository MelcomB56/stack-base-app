...
import { ArrowLeft, Loader2, Save, Tag, Layers, Cpu, FolderOpen } from "lucide-react";
    ...
    <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 20, maxWidth: 720 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={PANEL}>
          ...
          <Field label="Name *"><DSInput name="name" required placeholder="z.B. AzubiSuite" maxLength={100} /></Field>
          <Field label="Kurzbeschreibung *"><DSInput name="shortDesc" required placeholder="Was macht diese App? (min. 10 Zeichen)" maxLength={255} /></Field>
        ...
        <div style={PANEL}>
          ...
          <Field label="Produktion"><DSInput name="urlProd" type="url" placeholder="https://app.example.de" /></Field>
          <Field label="Staging"><DSInput name="urlStaging" type="url" placeholder="https://staging.example.de" /></Field>
          <Field label="Repository"><DSInput name="repoUrl" type="url" placeholder="https://github.com/org/repo" /></Field>
        </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {options.categories.map((cat) => (
                    <ChipToggle key={cat.id} label={cat.name} color={cat.color}
                  ...
                  {Object.entries(techGroups).map(([cat, items]) => (
                    <div key={cat}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                        {items.map((tech) => (
                          <ChipToggle key={tech.id} label={tech.name} color="#7C3AED"
              ...
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {options.tags.map((tag) => (
                    <ChipToggle key={tag.id} label={tag.name} color={tag.color}
        ...
        <div style={PANEL}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="Ansprechpartner"><DSInput name="contactName" placeholder="Name" maxLength={100} /></Field>
            <Field label="Support E-Mail"><DSInput name="supportEmail" type="email" placeholder="support@example.de" maxLength={200} /></Field>
          </div>
        </div>
